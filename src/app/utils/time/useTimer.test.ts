...
import { useCountdown, useTimer } from "./useTimer";
...
describe("useCountdown", () => {
	...
	it("should pause and resume countdown", () => {
		const { remaining, isActive, start, pause, resume } = useCountdown({
			duration: 5000,
		});
	...
	it("should stop countdown", () => {
		const { remaining, isActive, start, stop } = useCountdown({
			duration: 5000,
		});
	...
	it("should reset countdown", () => {
		const { remaining, isActive, isExpired, start, reset } = useCountdown({
			duration: 5000,
		});
