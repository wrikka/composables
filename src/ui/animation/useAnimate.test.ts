...
describe('useAnimate', () => {
  ...
  it('should start animation immediately by default when target is available', () => {
    ...
    expect(window.HTMLElement.prototype.animate).toHaveBeenCalledWith(keyframes, options)
  ...
  it('should play the animation', () => {
    const { play } = useAnimate(target, keyframes, { ...options, immediate: false })
    play()
    expect(window.HTMLElement.prototype.animate).toHaveBeenCalledWith(keyframes, options)
  })

  it('should update isPlaying and isFinished refs', () => {
    const { isPlaying, isFinished, play } = useAnimate(target, keyframes, { ...options, immediate: false })
