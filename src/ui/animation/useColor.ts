import { computed, type MaybeRef, type Ref, ref } from "vue";
...
export function useColor(
	...
	const { resume, pause } = useRafFn(
		(timestamp) => {
			...
			r.value =
				fromRgb.value[0] + (toRgb.value[0] - fromRgb.value[0]) * easedProgress;
			g.value =
				fromRgb.value[1] + (toRgb.value[1] - fromRgb.value[1]) * easedProgress;
			b.value =
				fromRgb.value[2] + (toRgb.value[2] - fromRgb.value[2]) * easedProgress;
	...
	return computed(
		() =>
			`rgb(${Math.round(r.value)}, ${Math.round(g.value)}, ${Math.round(b.value)})`,
	);
}
