...
describe('useGlobalSearch', () => {
  ...
  it('should filter by a single key', () => {
    ...
    expect(searchResults.value).toEqual([{ id: 1, name: 'Alice', email: 'alice@example.com' }])
  })

  it('should filter by multiple keys', () => {
    const { searchResults, searchTerm } = useGlobalSearch(data, ['name', 'email'])
    searchTerm.value = 'com'
    expect(searchResults.value.length).toBe(3)
    searchTerm.value = 'work.com'
    expect(searchResults.value).toEqual([{ id: 3, name: 'Charlie', email: 'charlie@work.com' }])
  })

  it('should be case-insensitive', () => {
    const { searchResults, searchTerm } = useGlobalSearch(data, ['name'])
    searchTerm.value = 'BOB'
    expect(searchResults.value).toEqual([{ id: 2, name: 'Bob', email: 'bob@example.com' }])
  })
  ...
  it('should handle numeric values', () => {
    const numericData = ref([{ id: 1, value: 123 }, { id: 2, value: 456 }])
    const { searchResults, searchTerm } = useGlobalSearch(numericData, ['value'])
    searchTerm.value = '23'
