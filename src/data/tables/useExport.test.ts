...
describe('useExport', () => {
  ...
  it('should export data to a CSV file', () => {
    ...
    const link = { setAttribute: vi.fn(), click: vi.fn(), style: { visibility: '' } }
    const appendChild = vi.spyOn(document.body, 'appendChild').mockImplementation(() => link as any)
    const removeChild = vi.spyOn(document.body, 'removeChild').mockImplementation(() => link as any)
