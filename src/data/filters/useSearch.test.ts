...
import { useAdvancedSearch, useSearch } from "./useSearch";
...
describe("useSearch", () => {
	...
	it("should handle case sensitivity", () => {
		...
		const { filteredItems: caseInsensitive } = useSearch(users, {
			caseSensitive: false,
		});
		search("ALICE");
	...
	it("should manage search keys dynamically", () => {
		const { filteredItems, search, setKeys, addKey, removeKey } = useSearch(
			users,
			{ keys: ["name"] },
		);
...
describe("useAdvancedSearch", () => {
	...
	it("should remove filters", () => {
		const { filteredItems, addFilter, removeFilter } =
	...
	it("should clear all filters and search", () => {
		const { filteredItems, search, addFilter, clearFilters, clear } =
			useAdvancedSearch(products);
