...
export const email = (message = 'Invalid email address'): FormValidationRule<string> => ({
...
export const minLength = (min: number, message?: string): FormValidationRule<string> => ({
  validate: (value: string) => (value && value.length >= min) || (message ?? `Must be at least ${min} characters`),
  message: message ?? `Must be at least ${min} characters`,
})

export const maxLength = (max: number, message?: string): FormValidationRule<string> => ({
  validate: (value: string) => (value && value.length <= max) || (message ?? `Must be ${max} characters or less`),
  message: message ?? `Must be ${max} characters or less`,
})

export const sameAs = (otherValue: any, otherName: string): FormValidationRule => ({
  validate: (value: any) => value === otherValue || `Does not match ${otherName}`,
