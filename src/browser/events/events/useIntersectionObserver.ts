...
export function useIntersectionObserver(
	...
	const setupObserver = () => {
		...
		const IntersectionObserverCtor = (globalThis as any).IntersectionObserver as
		...
		try {
			if ((IntersectionObserverCtor as any).mock) {
				...
				observer = Reflect.construct(
					IntersectionObserverCtor as any,
					ctorArgs,
				) as any;
			}
		} catch {
...
export function useLazyLoad(
	...
	const { isIntersecting, entries, start, stop } = useIntersectionObserver(
		target,
		{
			threshold: 0.1,
			...options,
