...
export function useScroll(
  ...
  const onScroll = (event: Event) => {
    const el = event.target === document ? window : (event.target as HTMLElement)
    ...
    const clientWidth = 'clientWidth' in el ? el.clientWidth : window.innerWidth
    const clientHeight = 'clientHeight' in el ? el.clientHeight : window.innerHeight
    const scrollWidth = 'scrollWidth' in el ? el.scrollWidth : document.body.scrollWidth
    const scrollHeight = 'scrollHeight' in el ? el.scrollHeight : document.body.scrollHeight
